...
const ImagesViewer: React.FC<ImageViewerProps> = ({ className, advertImages }) => {
    ...
    const images = useMemo(() => imagesData.images.map((image, index) => (
        <img loading="lazy" key={index}
             ...
             src={APP_ENV.REMOTE_MEDIUM_IMAGES_URL + image}
             onMouseDown={() => { setImagesData({ ...imagesData, currentImage: image }) }} />

    )) || [], [imagesData.images,imagesData.currentImage])

    const imagesPaths = useMemo(() => imagesData.images.map(x => APP_ENV.REMOTE_LARGE_IMAGES_URL + x) || [], [imagesData.images])
            ...
            {imagesData.images.length > 0 &&
                <div className={`${className} grid grid-cols-[18%,_1fr] gap-4 aspect-[1/1] h-full`}>
                    ...
                    <div className="h-full overflow-hidden">
                        <Image.PreviewGroup
                            items={imagesPaths}>
                            <Image loading='lazy' className="self-center object-cover " width={"100%"} height={"100%"}  src={APP_ENV.REMOTE_LARGE_IMAGES_URL + imagesData.currentImage} />
                        </Image.PreviewGroup>
                    </div>
                </div>
            }
